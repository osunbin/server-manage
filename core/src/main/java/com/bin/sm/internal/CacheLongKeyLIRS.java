 ...
 * segment.
 *
 *   LIRS 使用 IRR(Inter-Reference Recency) 来表示记录数据块访问历史信息，
 *   IRR 表示最近连续访问同一个数据块之间访问其他不同数据块非重复个数
 *
 *   (1) 2 3 4 3 (1) 5 6 5
 *   Cache1 的 IRR=3，连续两次访问 Cache1 之间访问 2,3,4，3 虽然访问两次但这里只算一次,
 *   一个数据块从上一次访问到当前时间访问其他不同数据块非重复个数称为这个数据块的 recency
 *
 *    lir - 热数据
 *    hir - 冷数据
 *         hir  1、有部分数据记录了它们的历史信息并标记为未驻留在缓存中 - nonresident-HIR
 *              2、部分驻留在缓存中的数据块称为 resident-HIR
 *
 *    LIRS 会限制 LIR 集合大小，保证所有的 LIR 数据块在缓存中，当发生缓存块淘汰时候，
 *    只会淘汰 HIR 集合中的数据块，这里用变量 Llirs 表示 LIR 集合大小，
 *    Lhirs 表示 HIR 集合中驻留在内存的数据块（resident-HIR）大小，
 *    用 L 表示驻留在缓存中的数据块总数：
 *    L = Llirs + Lhirs
 *
 *
...
public class CacheLongKeyLIRS<V> {
    ...
    public CacheLongKeyLIRS(Config config) {
        ...
        if (Integer.bitCount(config.segmentCount) != 1)
            throw  new IllegalArgumentException("The segment count must be a power of 2, is "+ config.segmentCount);
    ...
    public void setMaxMemory(long maxMemory) {
        if (maxMemory <= 0)
            throw  new IllegalArgumentException("Max memory must be larger than 0, is " + maxMemory);
     ...
     *  热条目仅在堆栈中。冷条目在队列中，也可能在堆栈中。
     *  非驻留冷条目的值设置为null，并且在堆栈和非驻留队列中。
     *
     * @param <V> the value type
     */
